...
namespace Relay
{
	public class IsoBoardManager : MonoBehaviour
	{
        ...
        public List<Animal> currentBoardAnimals = new List<Animal>();
		...
		void LayoutObject(char boardObject, int x, int y)
		{
			if (animals.Contains(boardObject) || homes.Contains(boardObject))
			{
                ...
                GameObject instance = Instantiate(toInstantiate, new Vector3(x, 1f, y), toInstantiate.transform.rotation) as GameObject;
				instance.transform.SetParent(boardHolder);
